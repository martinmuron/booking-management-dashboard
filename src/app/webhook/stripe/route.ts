...
const snapshotSecret = config.stripe.webhookSecrets.snapshot;
...
export async function POST(request: NextRequest) {
  ...
  try {
    event = stripe.webhooks.constructEvent(payload, signature, snapshotSecret) as Stripe.Event;
  } catch (error) {
    await addWebhookLog({
  ...
  try {
    const result = await handleStripeEvent(event, 'snapshot');
    ...
    await addWebhookLog({
      ...
      status: 'error',
      message: 'Unhandled error while processing Stripe webhook (snapshot)',
      error: error instanceof Error ? error.message : 'Unknown error',
    });
